...
bool MpcImpedanceController::init(ros::NodeHandle& node_handle) {
  ...
  // }
    ...
    x_target_ = Eigen::MatrixXd::Zero(generalized_state_size_, 1);
    x_r_ = Eigen::MatrixXd::Zero(generalized_state_size_, 1);
    
    equilibrium_state_ = Eigen::MatrixXd::Zero(generalized_state_size_, 1);;
    u_ = Eigen::MatrixXd::Zero(control_size_, 1);
    u_r_ = Eigen::MatrixXd::Zero(control_size_, 1);
...
void MpcImpedanceController::start() {
    ...
    Q_(generalized_state_size_, generalized_state_size_) = 0.0;
    R_ = Eigen::MatrixXd::Identity(control_size_, control_size_);
...
void MpcImpedanceController::castMPCToQPGradient() {
  // x_ref_ is alwayas zero here
  Eigen::VectorXd x_ref = Eigen::MatrixXd::Zero(generalized_state_size_, 1);
...
void MpcImpedanceController::updateConstraintVectors() {
  ...
  xMax_.head(2 * basic_state_size_) << state_limit - equilibrium_state_;
  for (size_t i = 0; i < horizon_+1; i++) {
...
void MpcImpedanceController::updateModelMatrix() {
  matrix_a_.block(generalized_state_size_, 0, 1, basic_state_size_) = -x_r_.head(basic_state_size_).transpose() * cartesian_stiffness_;
  matrix_a_.block(generalized_state_size_, basic_state_size_, 1, basic_state_size_) = -x_r_.segment(basic_state_size_, basic_state_size_).transpose() * cartesian_damping_; 
  matrix_b_.block(generalized_state_size_, 0, 1, basic_state_size_) = -x_r_.segment(basic_state_size_, basic_state_size_).transpose();
...
void MpcImpedanceController::update() {
  ...
  Eigen::VectorXd predict_x = QPSolution_.block(1, 0, generalized_state_size_, 1);
  ...
  // Eigen::VectorXd opti_value_mpc = (x_-x_ref_).transpose() * Q_ * (x_-x_ref_) + u_.transpose() * R_ * u_;
  // simModelUpdate();
  std::lock_guard<std::mutex> position_d_target_mutex_lock(
      error_state_mutex_);
  x_r_.head(generalized_state_size_-1) = x_target_.head(generalized_state_size_-1);
  x_r_(generalized_state_size_-1) =  x_r_(generalized_state_size_-1) + predict_x(generalized_state_size_-1);
  x_ = x_r_;
...
void MpcImpedanceController::simModelUpdate() {
  // can not use
  x_target_ = matrix_ad_ * x_ + matrix_bd_ * u_;
}
...
void MpcImpedanceController::errorStateCallback(const franka_msgs::ErrorStateConstPtr& msg) {
    std::lock_guard<std::mutex> error_state_mutex_lock(
      error_state_mutex_);
    for (size_t i = 0; i < 2*basic_state_size_ - 1; i++) {
        x_target_[i] = msg->error_state[i];
        equilibrium_state_[i] = msg->equilibrium_state[i];
    }
 }

void MpcImpedanceController::publishMPCCommand() {
    if (mpc_command_pub_.trylock()) {
      mpc_command_pub_.msg_.force.x = u_[0] + u_r_[0];
      mpc_command_pub_.msg_.force.y = u_[1] + u_r_[1];
      mpc_command_pub_.msg_.force.z = u_[2] + u_r_[2];
    }
