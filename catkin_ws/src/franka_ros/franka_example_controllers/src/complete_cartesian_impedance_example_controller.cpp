...
bool CompleteCartesianImpedanceExampleController::init(hardware_interface::RobotHW* robot_hw,
  ...
  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 1, &CompleteCartesianImpedanceExampleController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  sub_mpc_command_ = node_handle.subscribe(
      "mpc_command", 1, &CompleteCartesianImpedanceExampleController::mpcCommandCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
...
void CompleteCartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  // velocity_ << 0.005 * velocity_ + (1.0 - 0.005) * last_velocity_;
  ...
  u << accel_d_la_ - (PD - 1.0 * u_mpc_ - 1.0 * wrench_external_);
  //ROS_INFO("-------------------wrench_external_: %f-------------------", wrench_external_(2));
  v << jacobian_pinv * (u - djacobian * dq);

  // Cartesian PD control with damping ratio = 1
  tau_task << inertia * v - jacobian.transpose() * wrench_external_ * 0.0;
  ...
  velocity_d_ = velocity_d_target_ ;
...
void CompleteCartesianImpedanceExampleController::complianceParamCallback(
  ...
  cartesian_damping_target_.topLeftCorner(3, 3)
      << 0.8 * sqrt(2.0*config.translational_stiffness) * Eigen::Matrix3d::Identity();
...
void CompleteCartesianImpedanceExampleController::equilibriumPoseCallback(
  ...
  wrench_external_(2) = msg->wrench_sim.wrench.force.z;
  wrench_external_(1) = msg->wrench_sim.wrench.force.y;

  for (size_t i = 0; i < 3; i++) {
    predictReference[i] = position_d_target_[i];
    predictReference[i+3] = velocity_d_target_[i];
  }

  for (size_t i = 0; i < 90; i++) {
    predictReference[i+6] = msg->predictReference[i];
  }
}

void CompleteCartesianImpedanceExampleController::mpcCommandCallback(
    const franka_msgs::MPCStateConstPtr& msg) {
  std::lock_guard<std::mutex> mpc_mutex_lock(
      mpc_mutex_);
  u_mpc_target_.head(3) << msg->command.force.x, msg->command.force.y, msg->command.force.z;
}

void CompleteCartesianImpedanceExampleController::publishErrorState() {
  if (error_state_pub_.trylock()) {
    for (size_t i = 0; i < 3; i++) {
       error_state_pub_.msg_.pose[i] = position_[i];
       error_state_pub_.msg_.velocity[i] = velocity_[i];
    }
    for (size_t i = 0; i < 6; i++) {
       error_state_pub_.msg_.error_state[i] = error_state_[i];
    }
    for (size_t i = 0; i < 96; i++) {
       error_state_pub_.msg_.predict_reference[i] = predictReference[i];
    }
  }
