...
void MpcImpedanceController::equilibriumPoseCallback(const franka_msgs::ImpedanceControllerVariableConstPtr& msg) {
    std::lock_guard<std::mutex> error_state_mutex_lock(
      error_state_mutex_);
    equilibrium_state_.head(basic_state_size_) << msg->pose_d.pose.position.x, msg->pose_d.pose.position.y, msg->pose_d.pose.position.z;
    equilibrium_state_.tail(basic_state_size_) << msg->twist_d.twist.linear.x, msg->twist_d.twist.linear.y, msg->twist_d.twist.linear.z;
    state_ref_ = equilibrium_state_;
 }
